...
fn part2(input: &str) -> u16 {
    ...
    let new_b = evaluate("a", &instuction_map, &mut circuit);
    let new_instr_str = new_b.to_string() + " -> b";
    let new_instr = Instruction::from_line(new_instr_str.as_str());
    instuction_map.insert(new_instr.out_name(), new_instr);

    let mut circuit_rebuild = HashMap::new();
    evaluate("a", &instuction_map, &mut circuit_rebuild)
}
